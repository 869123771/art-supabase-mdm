<template>
  <ArtSectionCard
    class="work-center-navigator"
    title="生产范围"
    subtitle="先选车间，再定位工作中心"
    :loading="loading"
    :error="error"
    :empty="!loading && !error && !workshops.length"
    empty-title="暂无可用车间"
    empty-description="请先在“部门 / 产线”与“工作中心”中完成基础配置。"
    body-class="work-center-navigator__body"
    retryable
    @retry="$emit('refresh')"
  >
    <template #actions>
    ...
    <div class="work-center-navigator__workshop-field">
      <label for="workstation-workshop-select">车间 / 产线</label>
      <ElSelect
        id="workstation-workshop-select"
      ...
      >
        <ElOption
          ...
          :value="workshop.id"
        >
          <div class="work-center-navigator__workshop-option">
    ...
    <div class="work-center-navigator__section-heading">
      <ArtSectionTitle :show-line="false" :show-marker="false">工作中心</ArtSectionTitle>
      <span class="work-center-navigator__count">共 {{ workCenters.length }} 项</span>
    </div>
    ...
    <ElScrollbar v-if="filteredCenters.length" class="work-center-navigator__scroll">
      <div class="work-center-navigator__list">
        <button
          v-for="center in filteredCenters"
          :key="center.id"
          type="button"
          class="work-center-navigator__item"
        ...
        >
          <span class="work-center-navigator__item-icon" aria-hidden="true">
            <ArtSvgIcon icon="ri:dashboard-3-line" />
          </span>
          <span class="work-center-navigator__item-copy">
    ...
    <ArtEmptyState
      v-else
      class="work-center-navigator__empty"
      size="compact"
      :visual-size="72"
...
<script setup lang="ts">
  ...
  import type { WorkstationScopeCenter } from '@mdm/api'

  export interface WorkstationWorkshopOption {
    id: string
    name: string
    code: string
    path: string
  ...
  const props = defineProps<{
    workshops: WorkstationWorkshopOption[]
...
<style scoped lang="scss">
  .work-center-navigator {
    ...
    :deep(.work-center-navigator__body) {
      display: flex;
      flex: 1;
      flex-direction: column;
      gap: 12px;
